...
use leptos::{
    ...
    reactive::{
        ...
        traits::{
            DefinedAt, Get, GetUntracked, IsDisposed, Notify, Set, UntrackableGuard, Update, Write,
        },
    },
    server::ArcResource,
    suspense::Suspense,
...
impl<T> SsrSignalResource<T> {
    ...
    ///
    /// Once the above `Future` returns, the [`.set()`](Set::set)
    /// provided by the `Set` trait will be invoked with the return
    /// value, effecting the changes.
    ///
    ...
    /// With the provided fetcher and update function, generate a view
    /// to be added to a view tree that will apply the function that
    /// updates the value in place with the updater, using the
    ...
    /// will result in the expected reactivity.
    ///
    /// The updater will be invoked after provided `Future` generated by
    /// the `fetcher` returns.  The first argument is the same `&mut`
    /// that [`.update()`](Update::update) provided by the `Update`
    /// trait, with the second argument being the data produced by the
    /// `Future` that was generated from the `fetcher`, which should
    ...
    /// pub fn DocumentView() -> impl IntoView {
    ///     let document = expect_context::<ArcResource<Document>>();
    ///     let titles = expect_context::<SsrSignalResource<Vec<Title>>>();
    ///
    ///     view! {
    ///         {titles.update_with(
    ///             move || {
    ///                 let document = document.clone();
    ///                 async move {
    ///                     document.await.title
    ///                 }
    ///             },
    ///             // push the title of the just loaded document into the vec
    ///             |titles, new_title| titles.push(new_title),
    ///         )}
    ///         <div>
    ///             // Other components/elements.
    ///         </div>
    ...
    pub fn update_with<Fut, U>(
        ...
        updater: impl Fn(&mut T, U) + Send + Sync + 'static,
    ) -> impl IntoView
    where
        T: Clone + Send + Sync + 'static,
        ...
        let result = view! {
            <Suspense>{
                ...
                move || {
                    ...
                    Suspend::new(async move {
                        let value = fut.await;
                        this.inner_write_only().update(|v| {
                            updater(v, value);
                        });
                    })
                }
